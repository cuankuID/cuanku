<nav class="navbar navbar-expand-lg main-navbar">
    ...
    <li>
        <a href="{{ route('index.subscription.cart') }}" class="nav-link nav-link-lg "><i class="fas fa-shopping-cart"></i></a>
    </li>
    ...
    <li class="dropdown"><a href="#" data-toggle="dropdown" class="nav-link dropdown-toggle nav-link-lg nav-link-user">
        ...
        <div class="dropdown-menu dropdown-menu-right">
            <div class="dropdown-title">
                <small class="badge badge-info mx-1">{{ auth()->user()->role->name }}</small>
                @if (auth()->user()->subscription_id == null)
                @elseif (auth()->user()->subscription->name == 'Silver')
                <span class="text-muted">{{ auth()->user()->subscription->name }}</span>                    
                @elseif (auth()->user()->subscription->name == 'Gold')
                <span class="text-warning">{{ auth()->user()->subscription->name }}</span>
                @elseif (auth()->user()->subscription->name == 'Platinum')
                <span class="text-info">{{ auth()->user()->subscription->name }}</span>
                @endif
            </div>
            <a href="/dashboard/profile" class="dropdown-item has-icon">
                <i class="far fa-user"></i> Profile
            </a>
            <a href="{{ route('dashboard.list.subscription') }}" class="dropdown-item has-icon">
                <i class="fas fa-money-check"></i> Langganan
            </a>
        </div>
